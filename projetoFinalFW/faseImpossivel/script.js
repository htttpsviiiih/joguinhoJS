...
function atualizar() {
    ...
    for (const m of document.getElementsByTagName("moeda")) {
        ...
        if (jx < x + t && jx + tj > x && jy < y + t && jy + tj > y) {
            ...
            moedasPegas++;
            document.getElementById("placar").innerText = "Moedas: " + moedasPegas;
        }
    }
    ...
    if (
        ...
        jy + tj > chegada.getAttribute("py")
    ) {
        if (moedasPegas >= 7) {
            texto.innerText = "Parabéns, você concluiu a fase! Obrigada por jogar!";
              setTimeout(() => {
                window.location.reload();
            }, 2000);
            ...
            texto.innerText = "Você precisa coletar todas as moedas antes de chegar à área de chegada!";
        }
    }
}
